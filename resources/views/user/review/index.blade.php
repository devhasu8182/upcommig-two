 ...
 <div class="card shadow mb-4">
    ...
    <div class="card-body">
      <div class="table-responsive">
        @if(count($reviews)>0)
        <table id="example" class="table table-hover table-striped table-borderedss">
          <thead>
            <tr>
              <th>S.N.</th>
              <th>Review By</th>
            ...
            @foreach($reviews as $review)  
              ...
              @endphp
                <tr>
                    ...
                    <td>
                        <a href="{{route('user.productreview.edit',$review->id)}}" class="btn btn-primary btn-sm float-left mr-1" style="height:30px; width:30px;border-radius:50%" data-toggle="tooltip" title="edit" data-placement="bottom">
                          <span>E</span>
                        </a>
                        <form method="POST" action="{{route('user.productreview.delete',[$review->id])}}">
                          @csrf 
                          @method('delete')
                              <button class="btn btn-danger btn-sm dltBtn" data-id={{$review->id}} style="height:30px; width:30px;border-radius:50%" data-toggle="tooltip" data-placement="bottom" title="Delete">
                                D
                              </button>
                        </form>
                    </td>
